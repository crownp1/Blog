  ...
  <head>
	...
	<script type="text/javascript">
		...
		function fristPage(){
			...
			var blogid = document.rollForm.blogid.value;
			
			//搜索所需字段
			var blogtitle = document.rollForm.blogtitle.value;
			var date = document.rollForm.date.value;
			
			var url=document.rollForm.action+"&currPage=1&blogid="+blogid+"&blogtitle="+blogtitle+"&date="+date;;
			ajaxSubmit(url);
		...
		function previousPage(){
			...
			var blogtitle = document.rollForm.blogtitle.value;
			var date = document.rollForm.date.value;
			var url=document.rollForm.action+"&currPage="+currPage+"&blogid="+blogid+"&blogtitle="+blogtitle+"&date="+date;;
			ajaxSubmit(url);			
		}		
		//下一页
		function nextPage(){
		...
		function lastPage(){
			...
			var blogtitle = document.rollForm.blogtitle.value;
			var date = document.rollForm.date.value;
			var url=document.rollForm.action+"&currPage="+countPage+"&blogid="+blogid+"&blogtitle="+blogtitle+"&date="+date;;
			ajaxSubmit(url);
		}
		//去往第几页
		function gotoPage(_value){
			...
			var blogtitle = document.rollForm.blogtitle.value;
			var date = document.rollForm.date.value;
			var url=document.rollForm.action+"&currPage="+_value+"&blogid="+blogid+"&blogtitle="+blogtitle+"&date="+date;;
			ajaxSubmit(url);
		}
